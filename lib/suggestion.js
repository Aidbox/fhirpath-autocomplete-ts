    ...
    if (path.length > 0) {
        ...
        path.forEach((p) => {
            if (p.type === treeReducer_1.FHIRTokenType.Keyword) {
                if (p.value === "$index") {
                    schemaPath.push(new treeReducer_1.FHIRToken(treeReducer_1.FHIRTokenType.Type, "integer"));
                }
            }
            else {
                schemaPath.push(p);
...
function _suggest(fhirSchemas, type, forEachExpressions, externalConstants, fhirpath, cursor) {
    ...
    if (forEachExpressions.length > 0) {
        ...
        if (autocompleteContext.schemaPath.length === 0 &&
            autocompleteContext.token.type === treeReducer_1.FHIRTokenType.Empty) {
            ...
            schemaNode =
                resolvePath(fhirSchemas, fullSchemaPath.map((e) => e.value)) ?? parentNode;
        }
    }
    else {
